...
public class PaperweightRegen extends Regenerator {
     ...
     * From the thread that owns the temp world's chunks (its region thread on Folia, the
     * main thread on Paper): mark this regen's pyramid clean (the
                    ...
                    for (BlockVector2 chunk : chunks) {
                        for (int dx = -REGEN_PYRAMID_RADIUS; dx <= REGEN_PYRAMID_RADIUS; dx++) {
                            for (int dz = -REGEN_PYRAMID_RADIUS; dz <= REGEN_PYRAMID_RADIUS; dz++) {
                                ...
                                level.getChunkSource().removeRegionTicket(ChunkHolderManager.UNLOAD_COOLDOWN,
