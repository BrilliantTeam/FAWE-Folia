...
public class PaperweightRegen extends Regenerator {
     ...
     * tickets the chunk system backfills on every level-lowering removal, and drive the unload
     * ourselves from the thread that owns those chunks (the temp world's region thread on Folia,
     * the main thread on Paper -- FoliaLib picks), after marking this regen's pyramid
        ...
        try {
            ...
            FoliaLibHolder.getScheduler().runAtLocation(location, task -> {
                try {
                    for (BlockVector2 chunk : chunks) {
                        for (int dx = -REGEN_PYRAMID_RADIUS; dx <= REGEN_PYRAMID_RADIUS; dx++) {
                            for (int dz = -REGEN_PYRAMID_RADIUS; dz <= REGEN_PYRAMID_RADIUS; dz++) {
                                NewChunkHolder holder = level.chunkTaskScheduler.chunkHolderManager
                                        .getChunkHolder(chunk.x() + dx, chunk.z() + dz);
                                ChunkAccess loaded = holder == null ? null : holder.getCurrentChunk();
                                if (loaded == null) {
                                    continue;
                                }
                                loaded.setUnsaved(false);
                    ...
                    var holderManager = level.chunkTaskScheduler.chunkHolderManager;
                    // The chunk system's removeTicketAtLevel backfills a 1-tick UNKNOWN ticket
                    // whenever a removal lowers the chunk's ticket level, and delayed tickets only
                    // die in the holder manager's expiry tick -- which noSave temp worlds never run
                    // (the same gate that made UNLOAD_COOLDOWN immortal). Each tick() call decrements
                    // every delayed ticket in the region by one, so loop: 25 covers the UNKNOWN
                    // backfills (delay 1) plus the ~20-tick PLUGIN tickets this fork's
                    // getChunkAtAsync leaves on every chunk we read. On Folia tick() is
                    // region-scoped and requires region context.
                    for (int i = 0; i < 25; i++) {
                        holderManager.tick();
                    }
